...
void showHistory()
{
    ...
    if(recordSize == 0) maxPage = 1;
        ...
        maxPage = ceil((float)(recordSize) / RECORDS);
    ...
    while(flag)
    {
        ...
        Menu showHistory("SIMPLE BANK SIMULATION", *menu);
        showHistory.display(RECORDS + 5);
        ...
        for(shiftLines=0; shiftLines<RECORDS; shiftLines++)
        {
            /* move index for history usage */
            int index = pageIndex*RECORDS + shiftLines;
            ...
            cout << " |" << setw(11) << right << history[index].balance
                 << " |";
        }
...
void withdraw()
{
    ...
    if(balance <= -1000)
    {
        Menu withdrawMenu("SIMPLE BANK SIMULATION", withdrawForbidMenu_options);
...
WITHDRAW_ENTER:
    ...
    while(flag)
    {
        ...
        gotoXY(29, 6);
        cout << balance - withdrawCash;

        /* in debt warning */
        if(balance - withdrawCash < 0)
            cout << " [liability]";
...
void configurations()
{
    ...
    while(flag)
    {
        Menu configMenu("SIMPLE BANK SIMULATION", config_options, config_explanations);
        configMenu.display();
